...
class BlueprintTest extends TestCase
{
    ...
    public function it_sends_correct_parameters_when_creating_a_new_entity()
    {
        ...
        $this->manager->roles->create($attributes = [
            'name' => 'Acme Role',
            'title' => 'Foo Bar',
        ]);
    ...
    public function it_can_be_updated()
    {
        ...
        $company = $this->manager->roles(3)->update([
            'name' => 'Acme Role',
        ]);

        $this->assertInstanceOf(Entities\Role::class, $company);
        $this->assertInstanceOf(IdsEntity::class, $company);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('PUT', $request->getMethod());
            $this->assertEquals('roles/3', $request->getUri()->getPath());
    ...
    public function it_can_be_deleted()
    {
        ...
        $result = $this->manager->roles(2)->delete();
        $this->assertTrue($result);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('DELETE', $request->getMethod());
            $this->assertEquals('roles/2', $request->getUri()->getPath());
        });
    }
}
