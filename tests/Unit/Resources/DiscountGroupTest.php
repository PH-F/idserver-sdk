...
class DiscountGroupTest extends TestCase
{
    ...
    public function it_gets_all_discountgroups()
    {
        ...
        $collection = $this->manager->discountGroups->all();

        $this->assertInstanceOf(Collection::class, $collection);
        $this->assertCount(2, $collection);
        $this->assertInstanceOf(Entities\DiscountGroup::class, $collection->first());
    ...
    public function it_paginates_all_discountgroups()
    {
        ...
        $collection = $this->manager->discountGroups
            ->paginate(2, 1)
            ->all();
        ...
        $this->assertRequest(function (Request $request) {
            ...
            $this->assertEquals('discounts-groups', $request->getUri()->getPath());
    ...
    public function it_gets_just_one_discountgroup_by_id()
    {
        ...
        $item = $this->manager->discountGroups(1)->get();

        $this->assertInstanceOf(Entities\DiscountGroup::class, $item);
        $this->assertInstanceOf(IdsEntity::class, $item);
        $this->assertEquals(1, $item->id);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('GET', $request->getMethod());
            $this->assertEquals('discounts-groups/1', $request->getUri()->getPath());
        });
    }
    ...
    public function it_sends_correct_parameters_when_creating_a_new_discountgroup()
    {
        ...
        $this->manager->discountGroups->create($attributes = [
            'name' => 'Acme Promotion',
        ]);
    ...
    public function it_can_be_updated()
    {
        ...
        $company = $this->manager->discountGroups(3)->update([
            'name' => 'Acme Promotion',
        ]);

        $this->assertInstanceOf(Entities\DiscountGroup::class, $company);
        $this->assertInstanceOf(IdsEntity::class, $company);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('PUT', $request->getMethod());
            $this->assertEquals('discounts-groups/3', $request->getUri()->getPath());
    ...
    public function it_can_be_deleted()
    {
        ...
        $result = $this->manager->discountGroups(2)->delete();
