...
class VatRateTest extends TestCase
{
    ...
    public function it_gets_all()
    {
        ...
        $collection = $this->manager->vatRates->all();

        $this->assertInstanceOf(Collection::class, $collection);
        $this->assertCount(2, $collection);
        $this->assertInstanceOf(Entities\VatRate::class, $collection->first());
        $this->assertInstanceOf(IdsEntity::class, $collection->first());
        $this->assertEquals(2, $collection->last()->id);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('vat/rates', $request->getUri()->getPath());
    ...
    public function it_paginates_all_vat_vat_rates()
    {
        ...
        $collection = $this->manager->vatRates
            ->paginate(2, 1)
            ->all();
    ...
    public function it_gets_just_one_vat_rate_by_id()
    {
        ...
        $rate = $this->manager->vatRates(1)->get();

        $this->assertInstanceOf(Entities\VatRate::class, $rate);
        $this->assertInstanceOf(IdsEntity::class, $rate);
        $this->assertEquals(1, $rate->id);
    ...
    public function it_sends_correct_parameters_when_creating_a_new_vat_rate()
    {
        ...
        $this->manager->vatRates->create($attributes = [
            'name' => 'Acme Duration',
        ]);
    ...
    public function it_can_be_updated()
    {
        ...
        $vatRate = $this->manager->vatRates(3)->update([
            'name' => 'Acme Duration',
        ]);

        $this->assertInstanceOf(Entities\VatRate::class, $vatRate);
        $this->assertInstanceOf(IdsEntity::class, $vatRate);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('PUT', $request->getMethod());
            $this->assertEquals('vat/rates/3', $request->getUri()->getPath());
    ...
    public function it_can_be_deleted()
    {
        ...
        $result = $this->manager->vatRates(2)->delete();
        $this->assertTrue($result);

        $this->assertRequest(function (Request $request) {
            $this->assertEquals('DELETE', $request->getMethod());
            $this->assertEquals('vat/rates/2', $request->getUri()->getPath());
        });
    }
}
